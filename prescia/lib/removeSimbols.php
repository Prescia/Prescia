...
-*/
	...
	function removeSimbols($data,$remove_ext=false,$allowSpaces=false) {
	    ...
	    $b = 'aaaaaaaceeeeiiiidnoooooouuuuybsaaaaaaaceeeeiiiidnoooooouuuyybyRr'.(!$allowSpaces?"-":"");
	  	$data = utf8_decode($data);
	  	$data = strtr($data,utf8_decode($a),$b);
	  	$data = utf8_encode(strtolower($data));
		$l = strlen($data);
		$s = "";
		$allows = "qazwsxedcrfvtgbyhnujmikolp1234567890_-".($allowSpaces?" ":"").($remove_ext?"":".");
		for ($c=0;$c<$l;$c++)
